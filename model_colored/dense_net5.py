...
history = model.fit(x_train, y_train, epochs=40, batch_size=32, validation_data=(x_test, y_test))  # 에폭을 20으로 증가
end_time = time.time()
...
plt.legend()
# 파일로 저장
output_path = os.path.join("./", f'learning_curve5.png')
...
# 모델 저장
model.save('densenet_custom5.h5')
print("모델이 저장 완료")